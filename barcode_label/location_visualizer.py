...
class LocationVisualizer:
    def __init__(self, root):
        ...
        self.barcode_timeout = 0.1  # 100ms
        ...
        # 자동 바코드 리딩 안내
        barcode_info_label = tk.Label(main_frame, 
                                     text="💡 바코드 스캐너를 사용하면 자동으로 제품을 검색합니다.",
                                     font=("맑은 고딕", 9), fg="#4CAF50")
        barcode_info_label.pack(pady=2)
        ...
        # 검색 필드 선택
        self.search_field_var = tk.StringVar(value="제품코드")
    ...
    def on_key_press(self, event):
        ...
        if event.keysym in ['Control_L', 'Control_R', 'Alt_L', 'Alt_R', 'Shift_L', 'Shift_R', 
                           'Caps_Lock', 'Tab', 'Escape', 'F1', 'F2', 'F3', 'F4', 'F5', 'F6',
                           'F7', 'F8', 'F9', 'F10', 'F11', 'F12']:
            return
        
        # Ctrl+B 단축키 처리
        if event.state & 0x4 and event.keysym == 'b':  # Ctrl+B
            self.open_barcode_input()
            return
    ...
    def detect_barcode_pattern(self):
        ...
        # 제품 바코드 패턴 (숫자로만 구성, 길이 10-15자)
        if self.barcode_buffer.isdigit() and 10 <= len(self.barcode_buffer) <= 15:
            # 바코드 스캐너는 보통 빠르게 입력되므로 약간의 지연 후 처리
            self.root.after(50, lambda: self.process_barcode(self.barcode_buffer))
            self.clear_barcode_buffer()
    ...
    def process_barcode(self, barcode_data):
        ...
        # 88로 시작하는 13자리 제품 바코드
        if re.match(r'^88\d{11}$', barcode_data):
            if barcode_data in self.barcode_to_product:
                product_code = self.barcode_to_product[barcode_data]
                self.search_field_var.set("제품코드")
                self.search_var.set(product_code)
                self.apply_search()
                self.status_label.config(text=f"✅ 제품 바코드 인식: {barcode_data} → {product_code}", fg="#4CAF50")
            else:
                messagebox.showwarning("바코드 오류", f"등록되지 않은 제품 바코드입니다: {barcode_data}\n\n제품 정보 파일에 등록된 바코드만 사용 가능합니다.")
            return
        # 라벨 바코드 (제품코드-LOT-유통기한)
        ...
        if match:
            ...
            self.search_var.set(product_code)
            self.apply_search()
            self.status_label.config(text=f"✅ 라벨 바코드 인식: {barcode_data} → {product_code}", fg="#4CAF50")
            return
        # 기존 제품 바코드(기타)
        if barcode_data in self.barcode_to_product:
            product_code = self.barcode_to_product[barcode_data]
...
💡 단축키: Ctrl+B로 언제든지 바코드 리딩 창을 열 수 있습니다."""
    ...
    def on_cell_click(self, location, button):
        """셀 클릭 이벤트 처리 - 시각적 피드백 제공"""
        # 클릭된 버튼의 원래 상태 저장
        ...
        # 창이 포커스를 잃을 때도 복원하는 함수
        def on_focus_out(event):
            if not detail_window.winfo_exists():
                return
            # 약간의 지연 후 복원 (사용자가 다른 창을 클릭했을 때)
            self.root.after(100, restore_button_state)
        
        # 창 닫기 이벤트 바인딩
        detail_window.protocol("WM_DELETE_WINDOW", restore_button_state)
        
        # 창이 포커스를 잃을 때 이벤트 바인딩
        detail_window.bind("<FocusOut>", on_focus_out)
        
        # 안전장치: 2초 후 자동 복원 (창이 닫히지 않았을 경우)
        self.root.after(2000, lambda: self.safe_restore_button(button, original_bg, original_relief, 
                                                               original_text, original_font, original_fg))
        
        # 상세 정보 표시 (복원 콜백 전달)
        self.show_location_detail_in_window(location, detail_window, restore_button_state)
    
    def safe_restore_button(self, button, original_bg, original_relief, original_text, original_font, original_fg):
        """안전하게 버튼 상태를 복원하는 메서드"""
        try:
            # 버튼이 여전히 존재하는지 확인
    ...
    def show_location_detail_in_window(self, location, window, restore_callback):
        """지정된 창에 위치 상세 정보 표시"""
        if self.df.empty:
            restore_callback()
            return
        ...
        # Fix: Ensure location_df["제품명"] is a Series, not ndarray
    ...
    def show_statistics(self):
        ...
        if isinstance(self.df, pd.DataFrame):
            try:
                a_df = self.df[self.df["보관위치"].str.startswith("A")]["보관위치"]
                if isinstance(a_df, pd.Series):
                    a_locations = a_df.dropna().nunique()
                else:
                    a_locations = len(set(a_df))
            except Exception:
                a_locations = len(set(self.df[self.df["보관위치"].str.startswith("A")]["보관위치"]))
            try:
                b_df = self.df[self.df["보관위치"].str.startswith("B")]["보관위치"]
                if isinstance(b_df, pd.Series):
                    b_locations = b_df.dropna().nunique()
                else:
                    b_locations = len(set(b_df))
            except Exception:
                b_locations = len(set(self.df[self.df["보관위치"].str.startswith("B")]["보관위치"]))
        else:
            a_locations = 0
            b_locations = 0
        ...
        for zone_code, zone_data in self.zone_config["zones"].items():
            ...
            for row in range(sections["rows"]):
                ...
                for col in range(sections["columns"]):
                    ...
                    cell.bind("<Button-1>", lambda e, loc=location, btn=cell: self.on_cell_click(loc, btn))
